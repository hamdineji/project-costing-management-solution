...
@Directive({
  selector: '[ifDepartementPermission]'
})
export class IfDepartementPermissionDirective implements OnInit ,OnChanges {
  // the role the user must have
  @Input() public ifDepartementPermission: Array<string>;
  @Input() 
  ...
  public ngOnInit(): void {
      ...
      this.store.select(currentUserSelector).subscribe((data : any)=>{
        var permissions   = data.permissions
          ...
          if(permissions.findIndex(x=>x.name==this.ifDepartementPermission[0])!=-1){
